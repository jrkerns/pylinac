...
* :bdg-danger:`Change` Due to the changes in Gamma (see below), the profile is now the reference (fixed distribution) and the search distribution is now called evaluation
  (i.e. the first parameter changed name but the logic remains the same)

Gamma
^^^^^
* :bdg-danger:`Change` Realignment of notation: the gamma definitions are now the same as Low's paper, i.e. ``reference = measured = less dense``, ``evaluation = calculation = more dense``.
